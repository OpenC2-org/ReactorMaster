...
# TODO: These will change at somepoint, so putting them in variables for now
...
def respond_ack(modifiers):
	...
	"""
 	...
 	# If ID and response we can respond.
	if "id" in modifiers and command_respond_to_identifier in modifiers and "response" in modifiers:
		...
		# If upstream received without error
...
def respond_message(message,respond_to_url):
	...
	logger.info("Send response message to %s" % (respond_to_url))
